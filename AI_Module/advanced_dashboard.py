...
Upload your simulated or experimental data, choose a denoising filter or a trained AI model, visualize results, and compute accuracy.  
...
- Choose **Experimental** for real data with Estimated_alpha.
- Choose **Simulated (LSTM-ready)** for simulated data with Noisy_Signal.
""")
data_type = st.sidebar.radio("📁 Data Type", ["Experimental", "Simulated (LSTM-ready)"])
...
if uploaded_file:
    ...
    elif data_type == "Simulated (LSTM-ready)":
        st.sidebar.header("🤖 LSTM Inference")
        filter_type = "LSTM Prediction"
        seq_len = 20
        signal = df["Noisy_Signal"].values.reshape(-1, 1)
        X = create_sequences(signal, seq_len)
        X_tensor = torch.tensor(X, dtype=torch.float32)
        model = LSTMRegressor()
        model_path = "LSTM/models/lstm_model.pt"
        model.load_state_dict(torch.load(model_path))
        model.eval()
        with torch.no_grad():
            y_pred = model(X_tensor).numpy().flatten()
        y_full = np.full(len(df), np.nan)
        y_full[seq_len:] = y_pred
        df["Filtered_alpha"] = y_full
    ...
    signal_to_analyze = df["Filtered_alpha"].fillna(0)
    n = len(signal_to_analyze)
    dr = df["Distance_m"].iloc[1] - df["Distance_m"].iloc[0]
    freqs = np.fft.fftfreq(n, d=dr)
    fft_values = np.abs(np.fft.fft(signal_to_analyze))
    positive_mask = freqs >= 0

    fig_fft, ax_fft = plt.subplots(figsize=(10, 4))
    ax_fft.plot(freqs[positive_mask], fft_values[positive_mask], label=f"FFT of {filter_type} signal")
    ...
    if "Estimated_alpha" in df.columns:
    ...
    def generate_pdf():
        ...
        pdf.cell(0, 10, f"Data Type: {data_type}", ln=True)
